...
# Determine which port to use (HDMI-A-1 first, DP-1 fallback)
PORT_PATH=""
...
# Exit if neither port is connected (kinda weird but ok)
if [ -z "$PORT_PATH" ]; then
...
# Extract the Manufacturer Code and Display Product Name from the EDID
...
# Look up the full manufacturer name in the pnp.ids database
...
# Look for any line starting with the exact monitor name and prepend a hash to disable it
sed -i "s/^${ESCAPED_NAME}/#&/" "$CFG_FILE"

# Add the new autoset line
echo "${MONITOR_NAME}:1280x800@60 0 #autoset" >> "$CFG_FILE"
echo "Updated $CFG_FILE"

# Trigger gamescope to read the file and set the new resolution
